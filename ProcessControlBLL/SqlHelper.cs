...
namespace StudentMgr
{
    public static class SqlHelper
    {
        ...
        /// <param name="parameters"></param>
        /// <returns></returns>
        public static decimal ExecuteDecimal ( string column,string cmdText ,params SqlParameter [ ] parameters )
        {
            using ( SqlConnection conn = new SqlConnection ( Connstr ) )
            {
                conn . Open ( );
                DataTable dt= ExecuteDataTable ( conn ,cmdText ,parameters );
                if ( dt != null && dt . Rows . Count > 0 )
                    return string . IsNullOrEmpty ( dt . Rows [ 0 ] [ column ] . ToString ( ) ) == true ? 0 : Convert . ToDecimal ( dt . Rows [ 0 ] [ column ] . ToString ( ) );
                else
                    return 0;
            }
        }
